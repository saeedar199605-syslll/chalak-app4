...
export default function ManagementCenter({
  ...
  const updateCloudCredential = async (payload: Record<string, unknown>, method: 'POST' | 'DELETE' = 'POST') => {
    ...
    try {
      ...
      throw new Error('ارتباط با سرویس امن کلمه عبور برقرار نشد.');
    }
    const raw = await response.text();
    let result: { error?: string } = {};
    try { result = raw ? JSON.parse(raw) as { error?: string } : {}; }
    catch {
      if (import.meta.env.DEV) return result;
      throw new Error('پاسخ سرویس کلمه عبور معتبر نیست؛ Pages Functions را بررسی کنید.');
    }
    if (!response.ok) throw new Error(result.error || `ثبت امن کلمه عبور ناموفق بود (${response.status}).`);
    return result;
  };
  ...
  const handleSaveCustomPassword = async (e: React.FormEvent) => {
    ...
    if (!cleanPass) {
      ...
      if (resetSucceeded) setEditingPasswordEmp(null);
      return;
    }
